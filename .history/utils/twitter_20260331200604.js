...
export class TwitterAPI {
  ...
  async verifyRetweet(tweetId, userId) {
    try {
      // Use the retweeted_tweets endpoint to verify retweet status
      const data = await this.makeRequest(`/users/${userId}/retweeted_tweets?max_results=100`);
      return data.data && data.data.some(tweet => tweet.id === tweetId);
    } catch (error) {
  ...
  async verifyFollow(targetUserId, userId) {
    try {
      ...
      return data.data && data.data.some(follow => follow.id === targetUserId);
    } catch (error) {
